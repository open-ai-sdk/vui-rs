...
import { NativeTextWrap, RECT_FFI_BYTES, Status, TEXT_RUN_FFI_BYTES } from './native/ffi-symbols.ts'
...
export class VuiNode {
  ...
  setTextRuns(runs: TextRun[]): this {
    ...
    check(
      this.#lib.symbols.vui_node_set_text_runs(this.#ptr, this.id, runBytes, runs.length, bytes, bytes.byteLength),
      'set_text_runs',
    )
    return this
  }
