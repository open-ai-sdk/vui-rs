...
export class EditBuffer {
  ...
  constructor(value = "") {
    ...
    if (ptr === null)
      throw new Error("vui-core: failed to allocate EditBuffer");
    this.#ptr = ptr;
  ...
  getValue(): string {
    ...
    const copied = Number(
      this.#lib.symbols.vui_editbuf_copy_value(this.#ptr, out, out.byteLength),
    );
  ...
  setValue(value: string): void {
    ...
    check(
      this.#lib.symbols.vui_editbuf_set_value(
        this.#ptr,
        bytes,
        bytes.byteLength,
      ),
  ...
  insert(text: string): void {
    ...
    check(
      this.#lib.symbols.vui_editbuf_insert(this.#ptr, bytes, bytes.byteLength),
      "editbuf_insert",
    );
  }
  ...
  backspace(): void {
    check(
      this.#lib.symbols.vui_editbuf_backspace(this.#ptr),
      "editbuf_backspace",
    );
  }
  ...
  move(motion: EditMotionCode, selecting = false): void {
    check(
      this.#lib.symbols.vui_editbuf_move(this.#ptr, motion, selecting ? 1 : 0),
      "editbuf_move",
    );
  }
  ...
  selectAll(): void {
    check(
      this.#lib.symbols.vui_editbuf_select_all(this.#ptr),
      "editbuf_select_all",
    );
  }
  ...
  selectedText(): string {
    ...
    const out = new Uint8Array(len);
    const copied = Number(
      this.#lib.symbols.vui_editbuf_copy_selected(
        this.#ptr,
        out,
        out.byteLength,
      ),
  ...
  deleteSelection(): boolean {
    check(
      this.#lib.symbols.vui_editbuf_delete_selection(this.#ptr, this.#changed),
  ...
  undo(): boolean {
    check(
      this.#lib.symbols.vui_editbuf_undo(this.#ptr, this.#changed),
      "editbuf_undo",
    );
    return this.#changed[0] === 1;
  }
  ...
  redo(): boolean {
    check(
      this.#lib.symbols.vui_editbuf_redo(this.#ptr, this.#changed),
      "editbuf_redo",
    );
    return this.#changed[0] === 1;
  }
  ...
  cursor(): { row: number; col: number } {
    check(
      this.#lib.symbols.vui_editbuf_cursor(
        this.#ptr,
        this.#cursor,
        this.#cursor.subarray(1),
      ),
      "editbuf_cursor",
    );
