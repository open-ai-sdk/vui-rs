...
export const VuiDialogSelect = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function onKeyDown(ev: DispatchableEvent): void {
      ...
      switch (ev.name) {
        case "up": ev.preventDefault(); move(-1); break;
        case "down": ev.preventDefault(); move(1); break;
        case "pageUp": ev.preventDefault(); active.value = Math.max(0, active.value - props.maxRows); break;
        case "pageDown":
          ev.preventDefault();
    ...
    function labelSpans(label: string, indices: number[], on: boolean) {
      ...
      for (let i = 0; i < label.length; i++) {
        ...
        spans.push(
          h(
            "span",
            { fg: hit ? (on ? theme.selectedText : theme.primary) : undefined, bold: hit },
            label[i],
          ),
        );
      }
        ...
        () => [
          h(
            ...
            h(VuiInput, {
              ...
              "onUpdate:value": (v: string) => { query.value = v; active.value = 0; },
              onEnter: commit,
            }),
          ),
          h(
            VuiScrollBox,
            { scrollY: scrollY.value, maxHeight: props.maxRows, focusable: false, "onUpdate:scrollY": (y: number) => (scrollY.value = y) },
            { default: rows },
          ),
        ],
      );
