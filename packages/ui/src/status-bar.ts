...
// editor keeps pinned (cf. pi's `setFooter`/`setWidget`). `VuiStatusBar` is the
// core: a full-width, theme-coloured row with three slots — `left`, `center`,
...
export const VuiHeader = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    return () =>
      h(
        VuiStatusBar,
        { bg: props.bg ?? theme.backgroundElement, fg: props.fg ?? theme.text },
        slots,
      );
  },
});
