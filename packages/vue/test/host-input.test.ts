...
function mount(render: () => unknown) {
  ...
  return { app, ctx: app.context, cleanup: () => { app.unmount(); r.free(); } };
}
  ...
  test("a key dispatches to the focused node then bubbles to ancestors", () => {
    ...
    const { ctx, cleanup } = mount(() =>
      h("box", { onKeyDown: () => hits.push("parent") }, [
        h("box", {
          focusable: true,
          ref: (el: unknown) => { child = el as Renderable; },
    ...
    const { ctx, cleanup } = mount(() =>
      h("box", { onKeyDown: () => hits.push("parent") }, [
        h("box", {
          focusable: true,
          ref: (el: unknown) => { child = el as Renderable; },
          onKeyDown: (e: { preventDefault: () => void }) => { hits.push("child"); e.preventDefault(); },
        }),
      ]),
    );
    ctx.focusManager!.focus(child);
...
describe("<input> typing (JS edit model + v-model)", () => {
  ...
  test("maxLength caps insertion", async () => {
    const value = ref("");
    const { ctx, cleanup } = mount(() =>
      h(VuiHostInput, { value: value.value, focused: true, maxLength: 3, "onUpdate:value": (v: string) => (value.value = v) }),
    );
    await nextTick();
...
describe("EditRenderable model (unit)", () => {
  test("insert / move / delete operate on graphemes", () => {
    const { ctx, cleanup } = mount(() => h("input", { ref: (el: unknown) => ((globalThis as Record<string, unknown>).__e = el) }));
