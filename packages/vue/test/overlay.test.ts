...
describe('overlay layer + backdrop', () => {
  ...
  function modalApp(open = true) {
    return () => [
      ...
      open
        ? h('overlay', { backdrop: 0.5, alignItems: 'center', justifyContent: 'center' }, [
...
describe('overlay edge cases', () => {
  ...
  test('overlay accepts per-side inset overrides without crashing', async () => {
    const { renderer, ctx, app, cleanup } = mount(10, 5, () => [
      h('overlay', { top: 1, left: 2 }, [h('box', { width: 2, height: 1, bg: 0xcc0000ff })]),
    ])
    await nextTick()
    app.context.flushNow()
...
function isDescendant(
  root: import('../src/host/renderable.ts').Renderable,
  node: import('../src/host/renderable.ts').Renderable,
): boolean {
