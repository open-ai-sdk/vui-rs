...
describe("VuiCode render", () => {
  test("renders highlighted code with colored keyword", async () => {
    const { renderer, cleanup } = mount(24, 4, () =>
      h(VuiCode, { text: "const x = 1;", lang: "ts" }),
    );
    await nextTick();
  ...
  test("plaintext (no lang) still renders the source", async () => {
    const { renderer, cleanup } = mount(20, 3, () =>
      h(VuiCode, { text: "just words" }),
    );
    await nextTick();
