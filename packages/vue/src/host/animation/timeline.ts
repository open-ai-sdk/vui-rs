...
export function createAnimation(opts: AnimateOptions): Animation {
  ...
  const totalLoops =
    opts.loop === true ? Infinity : typeof opts.loop === 'number' ? Math.max(1, Math.floor(opts.loop)) : 1
...
export function createAnimationRegistry(onActive: (active: boolean) => void): AnimationRegistry {
