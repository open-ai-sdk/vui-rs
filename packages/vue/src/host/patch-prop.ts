...
function applyProp(el: Renderable, key: string, prev: unknown, next: unknown): void {
  ...
  if (el.kind === 'textarea' && applyTextarea(el as TextareaRenderable, key, next)) {
    el.markDirty()
    el.ctx.scheduleRender()
    return
  }
  ...
  if (LAYOUT_KEYS.has(key) || INSET_SIDES.has(key)) {
    ...
    if (el.kind === 'textarea') {
      ...
      if (key === 'width') textarea.textarea.autoWidth = next == null || next === 'auto'
      if (key === 'height') textarea.textarea.autoHeight = next == null || next === 'auto'
    }
...
function applyPaint(el: Renderable, key: string, next: unknown): boolean {
  ...
  switch (key) {
    ...
    case 'opacity':
      ...
      return true
    case 'overflow':
      p.overflow = next === 'hidden' ? 'hidden' : next === 'scroll' ? 'scroll' : 'visible'
      return true
    case 'zIndex':
    case 'z-index':
      p.zIndex = typeof next === 'number' && Number.isFinite(next) ? Math.trunc(next) : 0
      return true
    case 'backdrop':
      p.backdrop = parseBackdrop(next)
      return true
    case 'wrap':
      p.wrap = next === 'nowrap' || next === false ? 'nowrap' : next === 'char' ? 'char' : 'word'
...
function applyTextarea(el: TextareaRenderable, key: string, next: unknown): boolean {
  switch (key) {
    case 'value':
      {
...
function spreadStyle(el: Renderable, prev: Record<string, unknown> | null, next: Record<string, unknown> | null): void {
