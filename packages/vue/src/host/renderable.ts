...
export interface HostContext {
  ...
  /**
   * React to USER-initiated scroll (wheel/keys/scrollbar-drag — the scroll-box
   * `apply()` path). A settled selection is cleared; an in-progress drag captures
   * selected rows that are about to leave the viewport so copy can span more than
