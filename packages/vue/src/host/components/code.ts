...
import { type Highlighter, type StyledLine, createDefaultHighlighter, syntaxPaletteFromTheme } from '../highlighter.ts'
...
export const VuiCode = defineComponent({
  ...
  setup(props, { attrs }) {
    ...
    const themed = computed(() => createDefaultHighlighter(syntaxPaletteFromTheme(theme)))
    const lines = computed(() => (props.highlighter ?? themed.value).highlight(props.text, props.lang))

    return () => {
      const ls = lines.value
      const gutterWidth = String(ls.length).length
      const rows = ls.map((line, i) => {
        const content = h('text', { wrap: props.wrap, ...(props.lineNumbers ? { flexGrow: 1 } : {}) }, lineSpans(line))
