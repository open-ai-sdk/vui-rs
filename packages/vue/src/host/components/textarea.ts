  ...
  setup(props, { emit }) {
    ...
    function onKeyDown(ev: DispatchableEvent): void {
      ...
      switch (ev.name) {
        case "left":
          e.move(
            ev.ctrl || ev.alt ? EditMotion.WordLeft : EditMotion.Left,
            ev.shift,
          );
          break;
        case "right":
          e.move(
            ev.ctrl || ev.alt ? EditMotion.WordRight : EditMotion.Right,
            ev.shift,
          );
          break;
        case "up":
        ...
        case "home":
          e.move(
            ev.ctrl || ev.meta ? EditMotion.DocStart : EditMotion.Home,
            ev.shift,
          );
          break;
        case "end":
          e.move(
            ev.ctrl || ev.meta ? EditMotion.DocEnd : EditMotion.End,
            ev.shift,
          );
          break;
        case "backspace":
        ...
        case "tab":
          if (props.tabBehavior === "indent")
            e.insert(" ".repeat(Math.max(1, Math.floor(props.tabSize))));
          else handled = false;
          break;
        case "a":
