...
function renderTable(
  ...
  rowNodes.push(h('text', { fg: theme.muted, wrap: 'nowrap' }, widths.map((w) => '─'.repeat(w)).join('─┼─')))
  for (const row of table.rows) rowNodes.push(cellsRow(row, widths, theme, false))

  return h('box', { flexDirection: 'column', alignItems: 'stretch', ...margin }, rowNodes)
}

/** One table row as a single `<text>`: each cell's spans, padded, then a separator. */
function cellsRow(cells: MdSpan[][], widths: number[], theme: Theme, header: boolean): VNode {
