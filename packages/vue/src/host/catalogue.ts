// The JS-host element catalogue: maps a Vue tag to a `Renderable` factory. A
// custom-renderer `catalogue`/`extend` registry — `createElement(tag)` resolves
...
export function createRenderable(ctx: HostContext, tag: string): Renderable {
  ...
  if (
    ctx.renderer &&
    (node.kind === 'box' || node.kind === 'text' || node.kind === 'edit' || node.kind === 'textarea')
  ) {
