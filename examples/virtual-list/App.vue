<template>
  <box :width="{ pct: 1 }" :height="{ pct: 1 }" flexDirection="column"
    ...
    <box flexDirection="column" border="rounded" :borderColor="theme.border"
         :padding="{ left: 1, right: 1 }">
      <!-- Explicit height (rows) computed from the terminal size fills the screen. -->
      <VuiVirtualList :items="rows" :height="listHeight" :scrollbar="true" :focused="true"
    ...
    <text :fg="theme.textMuted">top row: <b :fg="theme.info">{{ top }}</b></text>
  </box>
</template>
