<template>
  ...
  >
    ...
    <text> </text>
    <text
      >Press <b :fg="theme.info">i</b> info · <b :fg="theme.success">s</b> success ·
      <b :fg="theme.warning">w</b> warning · <b :fg="theme.error">e</b> error
    </text>
...
function onKey(ev: DispatchableEvent): void {
  ...
  switch (ev.name) {
    case 'i':
      toast.show(`Info message #${++n}`, { kind: 'info' })
      break
    case 's':
      toast.show(`Saved successfully #${++n}`, { kind: 'success' })
      break
    case 'w':
      toast.show(`Warning: check your input #${++n}`, { kind: 'warning' })
      break
    case 'e':
      toast.show(`Error: something failed #${++n}`, { kind: 'error' })
      break
    case 'p':
      toast.show(`Sticky toast #${++n} — dismiss me`, { kind: 'info', duration: 0 })
      break
    case 'x':
      toast.clear()
      break
