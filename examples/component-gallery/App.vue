  ...
  >
    ...
    <box :flexGrow="1" flexDirection="row" :gap="2" :padding="{ left: 2, right: 2, top: 1, bottom: 1 }">
      ...
      <box flexDirection="column" :gap="0" :width="40">
        ...
        <text
          ><b :fg="theme.success">c</b> confirm · <b :fg="theme.success">a</b> alert ·
          <b :fg="theme.success">p</b> prompt</text
        >
        ...
        <box flexDirection="column" @keyDown="ac.onKeyDown">
          <box border="rounded" :borderColor="theme.border" :padding="{ left: 1, right: 1 }">
            <VuiInput
              ...
              @update:value="(v: string) => (acQuery = v)"
              @enter="onAcEnter"
            />
          </box>
          <VuiAutocomplete
      ...
      <box :flexGrow="1" flexDirection="column">
        ...
        <box flexDirection="column" border="rounded" :borderColor="theme.border" :padding="{ left: 1, right: 1 }">
          <VuiVirtualList
            ...
            :scrollbar="true"
            :scrollY="listScrollY"
            @update:scrollY="(y: number) => (listScrollY = y)"
          >
    ...
    <VuiDialogAlert
      ...
      title="Heads up"
      message="This is an alert dialog. Press Enter to dismiss."
    />
    <VuiDialogPrompt
      ...
      @submit="(v: string) => toast.show('Renamed to ' + v, { kind: 'success' })"
    />
    <VuiDialogSelect
...
const anyModal = computed(
  () => confirmOpen.value || alertOpen.value || promptOpen.value || selectOpen.value || paletteOpen.value,
)
...
const ac = useAutocomplete({
  ...
  providers: [
    (q) =>
      q.startsWith('@')
        ? PEOPLE.filter((p) => p.startsWith(q.slice(1))).map((p) => ({ label: '@' + p, value: p }))
        : [],
    (q) =>
      q.startsWith('/')
        ? SLASH.filter((s) => s.startsWith(q.slice(1))).map((s) => ({ label: '/' + s, value: s, hint: 'cmd' }))
        : [],
  ],
...
function onMenuKey(ev: DispatchableEvent): void {
  ...
  const scroll: Record<string, number> = {
    ...
    pageUp: -page,
    pageDown: page,
    home: -listScrollY.value,
    end: listMaxScroll() - listScrollY.value,
  }
