<template>
  <box :width="{ pct: 1 }" :height="{ pct: 1 }" flexDirection="column"
       ...
       :focusable="true" :focused="!anyOpen" @keyDown="onKey">
    ...
    <text> </text>
    <text>Press
      <b :fg="theme.success">c</b> confirm ·
      <b :fg="theme.success">a</b> alert ·
      <b :fg="theme.success">p</b> prompt ·
      <b :fg="theme.success">s</b> fuzzy select
    </text>
    <text> </text>
    ...
    <VuiDialogAlert v-model:open="alertOpen" title="Heads up"
                    message="This is an alert. Enter / Space / Esc to dismiss." @close="result = 'Alert dismissed'" />
    <VuiDialogPrompt v-model:open="promptOpen" title="Rename file" message="New file name:" placeholder="file.ts"
                     :validate="validateName" @submit="(v: string) => (result = 'Renamed to ' + v)" />
